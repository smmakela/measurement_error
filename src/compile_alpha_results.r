...
### Load results
####################################################################################
  fils1 <- list.files(resdir, pattern = "alpha_res_.*_nI_10_.*THIS.RData")
  fils2 <- list.files(resdir, pattern = "alpha_res_.*_nI_50_nj_2_.*THIS.RData")
  fils3 <- list.files(resdir, pattern = "alpha_res_.*_nI_50_nj_20_.*THIS.RData")
...
#cat(fil)
    ...
    df <- data.frame(alpha_res, model, n_I, n_j, simno, param = rownames(alpha_res))
...
today <- Sys.Date()
today <- sub("-", "_", today)
save(allres, file = paste(resdir, "/combined_alpha_res_", today, ".RData", sep = ""))
print(str(allres))
